...
class LandingHandler(RequestHandler):
    def get(self):
        self.write('<html><p>Hello, world! <a href="%s">Dashboard</a></p></html>' % self.reverse_url("dashboard"))

class IndexHandler(RequestHandler):
    def get(self):
        template = env.get_template('embed.html')
        script = server_document('http://localhost:5006/bkapp')
        self.write(template.render(script=script, framework="Tornado"))
...
server = Server({'/bkapp': bkapp}, num_procs=4, extra_patterns=[('/', LandingHandler, {}, "landing"),
                                                                ('/bokeh_dashboard', IndexHandler, {}, "dashboard")
                                                               ])
server.start()
